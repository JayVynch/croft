                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex justify-between items-center">
                            ...
                            All({{ $question->count() }}) | 
                                ...
                                <a href="#" class="text-blue-400 hover:text-blue-600">Published({{ $question->where('status','approved')->count() }})</a> |
                                <a href="#" class="text-blue-400 hover:text-blue-600">Pending({{ $question->where('status','pending')->count() }})</a> | 
                                <a href="#" class="text-blue-400 hover:text-blue-600">private({{ $question->where('question_type','private')->count() }})</a>
                            </div>
                            <div class="flex">
                            ...
                            <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                                ...
                                <div class="flex ">
                                    <div class="flex flex-col items-center justify-center space-x-3 px-2">
                                        <div class="">
                                            <img class="h-12 w-12 rounded-full" src="https://ui-avatars.com/api/?name={{ $question->user->name }}" alt="">
                                        </div>
                                    ...
                                    <div class="w-full ml-4">
                                        ...
                                        <div class="mt-6 flex flex-col justify-between">
                                            <div class="flex justify-between mb-2">
                                                ...
                                                <div>
                                                    This question is 
                                                    <form action="{{ route('update.stage') }}" method="post">
                                                        @csrf
                                                        <input type="hidden" name="question" value="{{ $question->id }}">
                                                        <select name="status" id="" class="border border-gray-500 px-2">
                                                            <option value="open" @if($question->stage == 'open') {{ 'selected' }} @endif >Open</option>
                                                            <option value="resolved" @if($question->stage == 'resolved') {{ 'selected' }} @endif>Resolved</option>
                                                            <option value="closed" @if($question->stage == 'closed') {{ 'selected' }} @endif>Closed</option>
                                                        </select>
                                                        <button class="px-4 py-1 text-white shadow rounded-lg bg-gray-400" type="submit">Change</button>
                                                    </form>
                                                </div>
                                            </div>
                                            <div class="flex w-full justify-between">
                                                <div class="px-6 py-4 whitespace-nowrap text-right w-full text-sm font-medium flex">
                                                    <div class="flex items-center">
                                                        status - <span class="text-green-500">{{ $question->status }} </span>
                                                    </div>
                                                    ...
                                                    <p class="px-2 w-full flex items-center">
                                                        ...
                                                        <span>{{ $question->answer->count() }} </span> <small>&nbsp;answers</small>
                                                    </p>
                                                    <p class="px-2 w-full flex items-center">
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 text-green-500">
                                                        ...
                                                        <span>{{ $question->votes->count() }}</span><small> &nbsp;votes</small>
                                                    </p>
                                                    <p class="px-2 w-full flex items-center">
                                                        <span>
                                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 text-green-500">
                                                        ...
                                                        <span>
                                                            {{ $question->views->count() }} <small>&nbsp;views</small>
                                                        </span>
                                                    </p>
                                                </div>
