        ...
        <main class="flex-1">
            <div class="py-6">
                <div class="max-w-7xl flex items-center mx-auto px-4 sm:px-6 lg:px-8 mb-4">
                    <h1 class="text-2xl font-semibold text-gray-900 mr-3">Answers</h1>
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 mb-4 lg:flex lg:items-center lg:justify-between">
                    <div class="flex-1 min-w-0">
                        <div class="flex justify-between items-center">
                            <div class="text-xs">
                                <a href="{{ route('admin.filter','all') }}" class="text-blue-400 hover:text-blue-600">All({{ $answers->count() }})</a> |
                                <a href="{{ route('admin.filter','published') }}" class="text-blue-400 hover:text-blue-600">Published({{ $answers->where('status','approved')->count() }})</a> |
                                <a href="{{ route('admin.filter','pending') }}" class="text-blue-400 hover:text-blue-600">Pending({{ $answers->where('status','pending')->count() }})</a> | 
                                <a href="{{ route('admin.filter','private') }}" class="text-blue-400 hover:text-blue-600">private({{ $answers->where('question_type','private')->count() }})</a>
                            </div>
                            <div class="flex">
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="bg-white">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    <a href="{{ route('show.answer',$answer->id) }}" > 
                                                        <span class="text-blue-400"> {{$answer->question->title }} </span>
                                                    </a>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    {{ $answer->user->name }}
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <a href="#" class="text-blue-400 hover:text-blue-600">Created - {{ $answer->created_at->diffForHumans() }}</a>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    {{ $answer->status }} 
                                                    <p>{{ $answer->count() }} - answers</p>
                                                    <p>
                                                        {{ $answer->votes->count() }} - voted
                                                    </p>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <form action="{{ route('approve.answer') }}" method="post">
                                                        @csrf
